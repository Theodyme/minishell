...
/*   Updated: 2023/07/26 17:55:54 by flplace          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			ft_bltin_where(t_cmd *cmd)
{
	...
	if (ft_strcmp(ft_str_tolower(tmp->str), "are") != 0 && ft_strcmp(ft_str_tolower(tmp->next->str), "we?") != 0 && ft_strcmp(ft_str_tolower(tmp->next->str), "we") != 0)
		printf("\033[34mᴛʀɪᴛᴏɴ\033[0m: Who are you talking to?\n");
	else
	{
		printf("\033[34mᴛʀɪᴛᴏɴ\033[0m: We're in the deep sea ! Specifically in the %s Ocean.\n", ft_currfile(pwd->value));
	}
	return (0);
}
